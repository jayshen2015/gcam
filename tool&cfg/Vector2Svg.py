...
def convertPaths(vdContainer,svgContainer,svgXml):  
    ...
    for vdPath in vdPaths:
        ...
        if vdPath.parentNode == vdContainer:
            ...
            if vdPath.hasAttribute('android:fillColor'):# and vdPath.attributes['android:fillColor'].value.find('#ffffff')<0
...
def convertVd(vdFilePath):
    ...
    # write xml to file
    svgXml.writexml(open(vdFilePath.replace(".xml", ".svg"), 'w'),indent="",addindent="  ",newl='\n')
